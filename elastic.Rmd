---
title: "Lasso Ridge Elastic Implementation"
author: "Huey"
...
I forked this from Bisaria's [Lasso Ridge Implementation|https://www.kaggle.io/svf/272035/c7674afa411b198f92611a43fe7d7c75/__results__.html#lasso-and-ridge-models]. Besides some minor edits
to the order or the confusion table and some notes, the original document is intact.
My work starts at "Kaggle - Elastic."

I tried using elastic net regularization to see if it would perform better than
lasso or ridge. Surprisingly, it did not. It's possible I'm doing something incorrectly,
but hopefully the code here could be useful for someone.
...
table(all[is.na(all$Age), "Pclass"])
```
...
When using ridge regression, it is suggested to scale the numeric columns. I've done this below, but didn't find it to have any effect on the actual results.
...
numericFeatures = c('Age', 'SibSp', 'Parch', 'Fare')

for (colName in numericFeatures) {
  # Males
  # Scale train
  cv.train.m[colName] = scale(cv.train.m[colName])
  center = attr(cv.train.m[[colName]], "scaled:center")
  scale = attr(cv.train.m[[colName]], "scaled:scale")
  
  # Use the same scale for validation and test sets
  cv.test.m[colName] = scale(cv.test.m[colName], center = center, scale = scale)
  test.male[colName] = scale(test.male[colName], center = center, scale = scale)
  
  # Females
  # Scale train
  cv.train.f[colName] = scale(cv.train.f[colName])
  center = attr(cv.train.f[[colName]], "scaled:center")
  scale = attr(cv.train.f[[colName]], "scaled:scale")
  
  # Use the same scale for validation and test sets
  cv.test.f[colName] = scale(cv.test.f[colName], center = center, scale = scale)
  test.female[colName] = scale(test.female[colName], center = center, scale = scale)
}
```
...
Below, we try elastic net regression. We use cross-validation to select the best
values of alpha and lambda.
...
Kappa is better here than ridge/lasso on the training set, but worse when looking 
...
Kappa is the same as validation (0.77) compared to ridge and lasso (0.77).
...
Elastic net regularization didn't seem to do any better, but as with anything, it's possible I could be doing something incorrectly. I hope this could be a good starting point for someone else.
